		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (item.isFormField() && !item.getFieldName().equals("flag")&& !item.getFieldName().equals("members") && !item.getFieldName().equals("agent")) {
					...
					System.out.println("图片的大小1:"+saveFile.length()+":@@@@@@@@@@@@@@@@@@@@@@@@@@@@图片的大小@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
			...
			if(null != flag && flag.equals("contractPhoto")){
				...
				LOGGER.info("跳转: "+PropertiesUtil.getPropertyValue("APP_CONTEXT")+ "/hhagent/html/ocrbackfile.html?img="+ img +"&timestamp="+dateString);
				response.sendRedirect(PropertiesUtil
						.getPropertyValue("APP_CONTEXT")
						+ "/hhagent/html/ocrbackfile.html?img="+ img +"&timestamp="+dateString);
			}else{
